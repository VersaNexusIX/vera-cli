...
def handle(args):
    if not args:
        return Fore.YELLOW + "⚠️ Link X/Twitter kosong." + Style.RESET_ALL
    ...
    ext = get_ext(choice)
    filename = get_next_filename(ext, prefix="X_")   # untuk x.py
    ...
    try:
        ...
        return Fore.GREEN + f"✅ X: Disimpan di: {final_path}" + Style.RESET_ALL

    except Exception as e:
        return Fore.RED + f"❌ Gagal unduh X: {e}" + Style.RESET_ALL
