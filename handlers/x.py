...
def handle(args):
    if not args:
        return Fore.YELLOW + "  ⚠️  Format: x <url> [--visible]" + Style.RESET_ALL
    url     = args[0]
    visible = "--visible" in args
    print(Fore.CYAN + "\n  📥  VERA X/Twitter Downloader")
    print(Fore.CYAN + "  ──────────────────────────────────────────")
    ...
    folder = get_download_path("VERA_X")
    fname  = get_next_filename(ext, prefix="X_", folder=folder)
    ...
    try:
        ...
        final = move_to_public_and_scan(fname, "X/Twitter") if visible else fname
        return Fore.GREEN + """
  ──────────────────────────────────────────
  ✅  Done
  Path     : """ + final + """
