...
def handle(args):
    if not args:
        return Fore.YELLOW + "  ⚠️  Format: ig <url> [--visible]" + Style.RESET_ALL
    url     = args[0]
    visible = "--visible" in args
    print(Fore.CYAN + "\n  📥  VERA Instagram Downloader")
    print(Fore.CYAN + "  ──────────────────────────────────────────")
    ...
    folder = get_download_path("VERA_Instagram")
    fname  = get_next_filename(ext, prefix="IG_", folder=folder)
    ...
    try:
        ...
        final = move_to_public_and_scan(fname, "Instagram") if visible else fname
        return Fore.GREEN + """
  ──────────────────────────────────────────
  ✅  Done
  Path     : """ + final + """
